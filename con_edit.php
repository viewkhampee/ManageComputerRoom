...
<link rel="stylesheet" href="https://code.jquery.com/ui/1.12.1/themes/base/jquery-ui.css">
    ...
    <div class="container-fluid page-body-wrapper">
     ...
     <?php include 'server.php';
      ...
      $cuser = "SELECT * FROM dd_member WHERE mem_user='$user'";
        ...
        $id_con = $_GET['id']; 
        $co = "SELECT * FROM contrac WHERE id_con = '$id_con'";
        $con = mysqli_query($conn,$co); 
        $contr = mysqli_fetch_array($con);  
        ?>
      <!-- partial -->
  ...
  ?>
                  <form action="update_con.php" method="post" id="dateForm" enctype="multipart/form-data" > 
                  <div class="col-md-12 mb-4">
                
                      <label for="exampleInputUsername1">ชื่อ-สกุล</label>
                      <input style="border-radius: 30px;"  type="text" name="name" value="<?php echo  $contr['name']; ?>" class="form-control" id="exampleInputUsername1" required>
                      <input style="border-radius: 30px;"  type="text" name="id_con" hidden value="<?php echo $id_con; ?>" class="form-control" id="exampleInputUsername1" required>
                 
                    </div>
                     <div class="col-md-12 mb-4">
                
                      <label for="exampleInputUsername1">เบอร์ติดต่อ</label>
                      <input style="border-radius: 30px;"  type="text" name="phone" value="<?php echo  $contr['phone']; ?>" OnKeyPress="return chkNumber(this)"  class="form-control" maxlength="10" minlength="10" autocomplete="off" required title="ต้องเป็นตัวตัวเลข และจำนวน 10 ตัวเลขเท่านั้น">
                 
                    </div>
                    <div class="col-md-12 mb-4">
                      <label for="exampleInputEmail1">สถานะ</label>
                    <select style="border-radius: 30px;"  class="form-control text-center" name="status" >
                <option <?php if($contr['status'] == 1){echo 'selected';} ?> value="1" >พร้อมใช้งาน</option>
                <option <?php if($contr['status'] == 0){echo 'selected';} ?> value="0" >ไม่พร้อมใช้งาน</option>
